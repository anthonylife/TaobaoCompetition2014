...
from tool import combineFeature, calSegNum, calSegNum1, calMonthLength, getTimeSegNode
...
def getUfeature(train_pairs, user_behavior):
    ...
    for pair in train_pairs:
        ...
        feature = [0 for i in xrange(4)]
        month_length = calMonthLength(month, day)
        time_seg_node = getTimeSegNode(month, day)
        product_set = [set() for i in xrange(len(time_seg_node)-1)]
        total_num = [0 for i in xrange(len(time_seg_node)-1)]
        for pid in user_behavior[uid]:
            for entry in user_behavior[uid][pid]:
                ...
                src_day = entry[2]
                if action_type == 1:
                    seg_num = calSegNum1(src_month, src_day, time_seg_node)
                    if seg_num >= 0:
                        product_set[seg_num].add(pid)
                        total_num[seg_num] += 1
        for i in xrange(len(product_set)-1):
            feature[0] += len(product_set[i])
            feature[2] += total_num[i]
        if (month_length - int(month_length)) != 0:
            feature[0] += len(product_set[-1])/(month_length - int(month_length))
            feature[2] += total_num[-1]/(month_length-int(month_length))
        else:
            feature[0] += len(product_set[-1])
            feature[2] += total_num[-1]
        feature[1] = feature[0]/month_length
        feature[3] = feature[2]/month_length
        if month_length < 1:
            feature[0] = len(product_set[0])/month_length
            feature[2] = total_num[0]/month_length
        else:
            feature[0] = len(product_set[0])
            feature[2] = total_num[0]

        features.append(feature)
    return features


def getPfeature(train_pairs, product_behavior):
    features = []
    for pair in train_pairs:
        pid = pair[1]
        month = pair[2]
        ...
        month_length = calMonthLength(month, day)
        time_seg_node = getTimeSegNode(month, day)
        user_set = [set() for i in xrange(len(time_seg_node)-1)]
        total_num = [0 for i in xrange(len(time_seg_node)-1)]
        for uid in product_behavior[pid]:
            for entry in product_behavior[pid][uid]:
                ...
                if action_type == 1:
                    seg_num = calSegNum1(src_month, src_day, time_seg_node)
                    if seg_num >= 0:
                        user_set[seg_num].add(uid)
                        total_num[seg_num] += 1
        for i in xrange(len(user_set)-1):
            feature[0] += len(user_set[i])
            feature[2] += total_num[i]

        if (month_length - int(month_length)) != 0:
            feature[0] += len(user_set[-1])/(month_length - int(month_length))
            feature[2] = total_num[-1]/(month_length-int(month_length))
        else:
            feature[0] += len(user_set[-1])
            feature[2] = total_num[-1]
        feature[1] = feature[1]/month_length
        feature[3] = feature[2]/month_length
        if month_length < 1:
            feature[0] = len(user_set[0])/month_length
            feature[2] = total_num[0]/month_length
        else:
            feature[0] = len(user_set[0])
            feature[2] = total_num[0]
...
def main():
    ...
    if para.tCF == 1:
        cf_feature = getCFfeature(train_pairs, user_factor, product_factor)
    ...
    if para.tP == 1:
        p_feature = getPfeature(train_pairs, product_behavior)
