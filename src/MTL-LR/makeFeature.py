...
###################################################################
# Date: 2014/4/10                                                 #
# Make feature for multi-task logistic regression method          #
...
from tool import getProductBuyClickRatio, getProductBuyBuyRatio, getUserBuy
from tool import getUserBuyClickRatio, getClusterLabelInfo, getAssociationRule
from tool import genAssociationRuleFeature, genFactorPref
...
def main():
    ...
    if para.file_num == 0:
        file_name = settings["MTLR_TRAIN_PAIR_FOR_VALIDATION"]
        writer = csv.writer(open(settings["MTLR_TRAIN_FILE"], "w"), lineterminator="\n")
        ...
        rule_file = settings["ASSOCIATION_RULE_FILE"]
        ...
        file_name = settings["MTLR_TRAIN_PAIR_FOR_TEST"]
        writer = csv.writer(open(settings["MTLR_TRAIN_FILE_FOR_SUBMIT"], "w"), lineterminator="\n")
        ...
        rule_file = settings["ASSOCIATION_RULE_FILE_FOR_SUBMIT"]
        data = [entry for entry in csv.reader(open(settings["TAR_DATA_FILE"]))]
        data = [map(int, entry) for entry in data[1:]]
    elif para.file_num == 2:
        file_name = settings["MTLR_TEST_PAIR_FOR_VALIDATION"]
        writer = csv.writer(open(settings["MTLR_TEST_FILE"], "w"), lineterminator="\n")
        cf_feature = getCFfeature(settings["CF_FEATURE_FILE"])
        user_cluster_file = settings["USER_CLUSTER_TEST_FILE"]
        item_cluster_file = settings["ITEM_CLUSTER_TEST_FILE"]
        rule_file = settings["ASSOCIATION_RULE_FILE"]
        ...
        file_name = settings["MTLR_TEST_PAIR_FOR_TEST"]
        writer = csv.writer(open(settings["MTLR_TEST_FILE_FOR_SUBMIT"], "w"), lineterminator="\n")
        cf_feature = getCFfeature(settings["CF_FEATURE_FILE_FOR_SUBMIT"])
        user_cluster_file = settings["USER_CLUSTER_TEST_FILE_FOR_SUBMIT"]
        item_cluster_file = settings["ITEM_CLUSTER_TEST_FILE_FOR_SUBMIT"]
        rule_file = settings["ASSOCIATION_RULE_FILE_FOR_SUBMIT"]
    ...
    #user_buy = getUserBuy(data)
    #association_rule, rule_num = getAssociationRule(rule_file)
    ...
    for i, pair in enumerate(csv.reader(open(file_name))):
        ...
        if uid in user_factor and pid in product_factor:
            output_feature[feature_idx] = genFactorPref(user_factor[uid], product_factor, pid, month, day, last_month, last_day, user_behavior[uid][0])
            output_feature[feature_idx+1] = 0
        else:
            output_feature[feature_idx] = 0
            output_feature[feature_idx+1] = -1
        ...
        '''if uid in user_factor:
            output_feature[feature_idx:feature_idx+20] = user_factor[uid]
        feature_idx += 20
        if pid in product_factor:
            output_feature[feature_idx:feature_idx+20] = product_factor[pid]
        feature_idx += 20'''
        ...
        # 11. association rule feature
        '''output_feature[feature_idx:feature_idx+rule_num] = genAssociationRuleFeature(user_buy[uid],
                association_rule, rule_num, pid)
        feature_idx += rule_num'''
