...
from tool import calSegNum, getDayDiff
...
def getHistorySet(user_history, month, day):
    ...
    for entry in user_history:
        ...
        if h_month < month:
            history_set.add(pid)
        elif h_month == month and h_day < day:
            history_set.add(pid)
    return history_set


def getHistorySet1(one_user_history, src_seg_num):
    history_set = set([])
    for entry in one_user_history:
        pid, month, day = entry[:3]
        seg_num = calSegNum(month, day)
        if seg_num == src_seg_num:
            history_set.add(seg_num)
    return history_set
...
def main():
    ...
    parser.add_argument('-c', type=int, action='store',
            dest='clean', help='wheter clean the noisy data')

    if len(sys.argv) != 7:
        print 'Command e.g.: python genPairs.py -t 0(1) -r 5 -c 0(1)'
        sys.exit(1)

    para = parser.parse_args()
    if para.target == 0:
        if para.clean == 1:
            data = [entry for entry in csv.reader(open(settings["TRAIN_CLEAN_DATA_FILE"]))]
        else:
        ...
        writer = csv.writer(open(settings["MTLR_TRAIN_PAIR_FOR_VALIDATION"], "w"), lineterminator="\n")
        writer.writerows(output_result)
    elif para.target == 1:
        if para.clean == 1:
            data = [entry for entry in csv.reader(open(settings["TAR_CLEAN_DATA_FILE"]))]
        else:
        ...
        #pairs, targets = popGenPair(data, para.ratio)
        output_result = [pair+target for pair, target in zip(pairs, targets)]
        writer = csv.writer(open(settings["MTLR_TRAIN_PAIR_FOR_TEST"], "w"), lineterminator="\n")
        writer.writerows(output_result)
    elif para.target == 2:
        if para.clean == 1:
            data = [entry for entry in csv.reader(open(settings["TRAIN_CLEAN_DATA_FILE"]))]
        ...
        writer = csv.writer(open(settings["MTLR_TEST_PAIR_FOR_VALIDATION"], "w"), lineterminator="\n")
        ...
        data = [map(int, entry) for entry in data[1:]]
        pairs = genTestPair(data, para.target)
        writer = csv.writer(open(settings["MTLR_TEST_PAIR_FOR_TEST"], "w"), lineterminator="\n")
        writer.writerows(pairs)
    else:
